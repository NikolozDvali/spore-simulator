...
class Character:
    def __init__(
        ...
        appendage_manager: AppendageManager = AppendageManager(),
    ) -> None:
        self.name = name
    ...
    def can_move(self, movement_protocol: type[Move], direction: Direction):
        ...
        if not self.stats_manager.can_use_stamina(movement_protocol.requires_stamina):
            return False
        if not self.position_manager.can_move_to(
            self.position + movement_protocol.speed * direction.value
        ):
            return False
        return True
    ...
    def move(self, movement_protocol: type[Move], direction: Direction):
        ...
        self.stats_manager.use_stamina(
            movement_protocol.uses_stamina, movement_protocol.requires_stamina
        )

    def get_available_move_protocols(self) -> list[type[Move]]:
        return [
            move_protocol
            for move_protocol in MOVE_PROTOCOLS
            if self.can_move(move_protocol, Direction.NOWHERE)
        ]
    ...
    def __getattr__(self, attr_name: str):
        stats_attributes = {
            ...
            "attack_power": self.stats_manager.calculate_attack_power(
                self.appendage_manager
            ),
